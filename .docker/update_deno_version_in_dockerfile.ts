...
let dockerfileContent = new TextDecoder().decode(await Deno.readFile("./.docker/drash.dockerfile"))
dockerfileContent =  dockerfileContent.replace(
    /-s v[0-9.]+[0-9.]+[0-9]/g,
    `-s v${latestDenoVersion}`
);
await Deno.writeFile("./.docker/drash.dockerfile", new TextEncoder().encode(dockerfileContent))