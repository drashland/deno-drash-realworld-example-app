import {
  clearTestArticles,
  clearTestComments, clearTestSessions,
  createServerObject,
  createTestArticle,
  createTestComment, createTestSession
} from "./utils.ts";
...
Rhum.testPlan("integration/article_comments_resource_test.ts", () => {
  Rhum.testSuite("GET /articles/:slug/comments", () => {
    Rhum.testCase("Responds with a 200 status when comments for an article exist", async () => {
      ...
      Rhum.asserts.assertEquals(body.data[0].author_username, "Test Username");
  ...
  Rhum.testSuite("POST /articles/:slug/comments", () => {
    ...
    //   await server.close()
    // });
    Rhum.testCase("Responds with 200 on valid post and saves the comment for the article", async () => {
      await server.run({ hostname: "localhost", port: 1447 });
  ...
  Rhum.testSuite("DELETE /articles/comments/:id", () => {
    Rhum.testCase("Responds with a 200 status when deleting a existing comment", async () => {
      await server.run({ hostname: "localhost", port: 1447 });
      ...
      const res = await fetch("http://localhost:1447/articles/comment/" + comment.id, {
        ...
        credentials: "same-origin",
        headers: {
          "Cookie": "drash_sess=" + session.session_one + "|::|" + session.session_two
        }
      });
      const body = await res.json();
