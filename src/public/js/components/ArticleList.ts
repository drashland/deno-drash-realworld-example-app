import { Component, html, reactive, ReadonlyReactiveValue, computed, ReactiveValue } from "./deps.ts"
import { fetchArticles, articles } from "../state.ts"
...
export class ArticleList extends Component {
    ...
    #author: ReactiveValue<string> | ReactiveValue<undefined> = reactive(this.author).bind(value => {
    ...
    this.#favorited = reactive<boolean, unknown>(this.favorited ?? false).bind(value => {
      this.#resetPagination()
      this.#fetchArticles()
    }, {
      noFirstRun: true,
    ...
    override template = this.html(html`
        ...
        `, html`
            ${computed(() => {
              ...
              return html`
                ${articles.value.map((article: any) => html`
                <${ArticlePreview}
