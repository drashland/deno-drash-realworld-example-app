import { Component, html, computed, reactive, classNames, ReactiveValue, swal } from "./deps.ts"
import { profile, isAuthenticated, deleteArticle, toggleArticleFavorite } from "../state.ts"
...
export class ArticleActions extends Component {
    ...
    #favoriteArticleLabel = computed(() => {
        return this.article.favorited.value ? "Unfavorite Article" : "Favorite Article";
    })
    ...
    override template = this.html(html`
        ...
        ${this.#canModify.truthy(html`
        <span>
            <a class="btn btn-sm btn-outline-secondary" href=${this.#editArticleLink}>
                <i class="ion-edit"></i> <span> Edit Article</span>
            </a>
            <span>  </span>
            <button class="btn btn-outline-danger btn-sm" on:click=${() => this.#deleteArticle()}>
        ...
        <span>
            <button class="btn btn-sm btn-outline-secondary" on:click=${() => this.#toggleFollow()}>
