...
export const comments = reactive([]) as unknown as ReactiveArray<Record<string, unknown>>;
...
export const setProfile = (newProfile: any) => {
  // @ts-ignore
  Object.entries(newProfile).forEach(([key, value]) => profile[key].value = value)
}
...
export const deleteComment = async ({ id, commentId }: { id: number | string, commentId: number}) => {
